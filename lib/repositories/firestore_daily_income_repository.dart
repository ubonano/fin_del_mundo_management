...
class FirestoreDailyIncomeRepository implements DailyIncomeRepository {
  ...
  Future<void> update(DailyIncome income) {
    _logger.info('Starting to update a daily income');
    return _collection
        .doc(income.id)
        .update(income.toDocument())
        .then((value) => _logger.info('Successfully updated a daily income'))
        .catchError((error) {
      _logger.severe('Failed to update a daily income: $error');
      throw error;
    });
  }
